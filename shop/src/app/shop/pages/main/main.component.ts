...
export class MainComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.categories$?.subscribe(res => {
      res.map((category) => {
        let subCategoryNum = Math.floor(Math.random()*category.subCategories.length);
        const subCategoryId = category.subCategories[subCategoryNum].id;
        this.productService.getProductsForSlider(category.id, subCategoryId).subscribe(item => {
